...
function renderError() {
  ...
  resultsSubtitle.textContent = "Проверьте соединение и попробуйте ещё раз.";
  resultsCount.textContent = "";
  resultsContent.innerHTML = `<div class="empty-state"><div class="empty-icon" aria-hidden="true">!</div><div><h3>Что-то пошло не так</h3><p>Не удалось связаться с сервисом подбора. Ваши параметры сохранены — повторите запрос.</p><button class="submit-button" id="retry-button" type="button"><span>Попробовать снова</span><svg viewBox="0 0 20 20" aria-hidden="true"><path d="M4 10h12m-5-5 5 5-5 5"/></svg></button></div></div>`;
